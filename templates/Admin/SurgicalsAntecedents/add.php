...
<div class="row">
    <aside class="column">
        <div class="side-nav">
            ...
            <?= $this->Html->link(__('Antecedentes Quirurgicos'), ['action' => 'index'], ['class' => 'side-nav-item btn btn-danger text-uppercase font-weight-bold']) ?>
        </div>
    </aside>
    <div class="column-responsive column-80 card mx-auto">
        <div class="medicalsAntecedents form content card-body">
            <?= $this->Form->create($surgicalsAntecedent, ['id' => 'form_antecedente']) ?>
            <fieldset>
                <legend class="text-uppercase font-weight-bold"><?= __('Antecedente Quirurgico') ?></legend>
                <?php
                    echo $this->Form->control('descripcion', ['placeholder' => 'Ingrese un antecedente quirurgico', 'class' => 'required']);
...
$('#form_antecedente').validate({
    ...
    messages: {
        descripcion: {
            required: "Por favor ingrese un antecedente quirurgico",
