...
<div class="medicalsAntecedents index content">
    ...
    <br>
    <h3 class="text-uppercase font-weight-bold"> <i class="fas fa-procedures"></i> <?= __('Antecedentes Quirurgicos') ?></h3>
        ...
        <table class="table table-dark table-bordered" style="border-radius: 15px 15px 15px 15px !important;" id="antecedentes">
            <thead class="thead thead-light">
                <tr>
                    ...
                    <th class="font-weight-bold text-uppercase">Antecedentes Quirurgicos</th>
                    <th class="text-center actions text-uppercase"><?= __('Opciones') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($surgicalsAntecedents as $surgicalsAntecedent): ?>
                <tr>

                    <td class="font-weight-bold"><?= h($surgicalsAntecedent->descripcion) ?></td>
                    <td class="pagination text-center">

                        <?= $this->Html->link(__(''), ['action' => 'edit', $surgicalsAntecedent->id], ['class' => 'fas fa-edit btn btn-warning']) ?>
                        <?= $this->Form->postLink(__(''), ['action' => 'delete', $surgicalsAntecedent->id], ['confirm' => __('¿Desea eliminar el antecedente {0}', $surgicalsAntecedent->descripcion.'?'), 'class' => 'fas fa-trash btn btn-warning elimi_antecedente']) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        ...
        cancelButtonText: 'Cancelar',
            ...
            if (result.value) {
                ...
                delete_form.submit();
                    ...
                    swal.fire(
                    'Cancelado',
                    'La eliminación del antecedente quirurgico, ha sido cancelado.',
                    'error'
                )
